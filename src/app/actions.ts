...
export type ActionState = { ok?: boolean; error?: string; publicId?: string; pin?: string; editToken?: string };
...
export async function updateWithPin(_: ActionState, form: FormData): Promise<ActionState> {
  try { const v=pinSchema.parse(formObject(form)); const {error}=await supabase().rpc("update_resource_status",{payload:v}); if(error) throw error; revalidatePath("/"); return {ok:true}; } catch{return {error:"PIN o enlace inválido. No se realizó ningún cambio."}}
}
